...
public class SearchActivity extends AppCompatActivity {
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		filter_icon.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				...
				FilterBottomdialogFragmentDialogFragmentActivityN.show(getSupportFragmentManager(),"1");
			}
		});
					...
					public void onDataChange(DataSnapshot _dataSnapshot) {
						...
						if (edittext1.getText().toString().equals("")) {
							if (data.getString("filter", "").equals("")) {
								...
								if (data.getString("filter", "").contains(_childValue.get("type of dormitory").toString())) {
									recyclerview1.setAdapter(new Recyclerview1Adapter(listmap));
									recyclerview1.setVisibility(View.VISIBLE);
									imageview1.setVisibility(View.GONE);
								}
								else {
									if (data.getString("filter", "").contains(_childValue.get("amenities").toString())) {
										recyclerview1.setAdapter(new Recyclerview1Adapter(listmap));
										recyclerview1.setVisibility(View.VISIBLE);
										imageview1.setVisibility(View.GONE);
									}
									else {
										if (data.getString("filter", "").contains(_childValue.get("inclusion").toString())) {
											recyclerview1.setAdapter(new Recyclerview1Adapter(listmap));
											recyclerview1.setVisibility(View.VISIBLE);
											imageview1.setVisibility(View.GONE);
										}
							...
							n = listmap.size() - 1;
							length = listmap.size();
							for(int _repeat76 = 0; _repeat76 < (int)(length); _repeat76++) {
	...
	public class Recyclerview1Adapter extends RecyclerView.Adapter<Recyclerview1Adapter.ViewHolder> {
		...
		public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
			...
			View _v = _inflater.inflate(R.layout.search_post, null);
		...
		public void onBindViewHolder(ViewHolder _holder, final int _position) {
			...
			if (_data.get((int)_position).containsKey("photo1")) {
				Glide.with(getApplicationContext()).load(Uri.parse(_data.get((int)_position).get("photo1").toString())).into(photo);
			}
			else {
