...
public class SearchUserActivity extends AppCompatActivity {
			...
			public void onChildAdded(DataSnapshot _param1, String _param2) {
				...
				users.addListenerForSingleValueEvent(new ValueEventListener() {
					...
					public void onDataChange(DataSnapshot _dataSnapshot) {
						...
						if (_childValue.containsKey("fullname")) {
							n = listmap.size() - 1;
							length = listmap.size();
							for(int _repeat50 = 0; _repeat50 < (int)(length); _repeat50++) {
								if (listmap.get((int)n).get("fullname").toString().toLowerCase().contains(edittext1.getText().toString().toLowerCase())) {
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			View _view = _v;
			if (_view == null) {
				_view = _inflater.inflate(R.layout.user_post, null);
			}
			
			final LinearLayout linear1 = _view.findViewById(R.id.linear1);
			final androidx.cardview.widget.CardView cardview1 = _view.findViewById(R.id.cardview1);
			final TextView fullname = _view.findViewById(R.id.fullname);
			...
			if (_data.get((int)_position).containsKey("profile")) {
				Glide.with(getApplicationContext()).load(Uri.parse(_data.get((int)_position).get("profile").toString())).into(profile);
			}
			else {
				
			}
			if (_data.get((int)_position).containsKey("fullname")) {
				fullname.setText(_data.get((int)_position).get("fullname").toString());
			}
			else {
			...
			linear1.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View _view) {
					...
					intent.putExtra("seconduser", _data.get((int)_position).get("userKey").toString());
					intent.putExtra("getPost", "");
